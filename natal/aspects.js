/* ============================================================
   ZODIAC · ASPETTI NATALI — pianeta↔pianeta dentro la carta
   Ispirato al report di Cafe Astrology (tabella aspetti con
   valore positivo/negativo), ma in voce pop.
   Dipende da: window.NATAL
   API: window.NATALASPECTS.compute(result) -> [{a,b,aspect,orb,kind,score,text}]
        window.NATALASPECTS.balance(result) -> {el,mod,pol}
   ============================================================ */
(function () {
  const ASPECTS = [
  ...
  const ROLE = {
    ...
    Mars: "la tua grinta", Jupiter: "la tua espansione", Saturn: "la tua disciplina",
    Uranus: "la tua libertà", Neptune: "il tuo sogno", Pluto: "la tua intensità",
    Ascendant: "la tua immagine", MC: "la tua direzione"
  };
  const QUALITY = {
    intenso: "si fondono e si amplificano: energia concentrata, nel bene e nel male",
    armonico: "scorrono insieme con facilità: un talento naturale su cui contare",
    teso: "si sfregano e si sfidano: attrito che, se lavorato, diventa forza"
  };
  ...
  window.NATALASPECTS = { compute, balance };
})();
